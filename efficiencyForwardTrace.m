...
for numWavelengths = 20 : 20 : 100
    efficiency = zeros(1,10);
    efficiencyIndex = 1;
    for sampleIndex = 1:numSamples
        waveguideLength = lengthList(sampleIndex);
        zspan = [0 waveguideLength];
        coupledInPower_W = powerPerWavelength_W * numWavelengths;
        coupledInIntensity = coupledInPower_W / Aeff;  % W/cm2

        [z,intensity] = ode45(ode,zspan,coupledInIntensity);
        coupledOutPower_W = intensity(length(intensity))*Aeff;
...
figure(1)
title("transmission efficiency with fixed INPUT power")
